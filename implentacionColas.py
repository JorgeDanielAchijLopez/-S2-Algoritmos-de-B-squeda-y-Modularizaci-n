...
class InteractiveSystem:
    def __init__(self):
        ...
        self.customer_service = {"vip": Queue(), "regular": Queue()}
    ...
    def add_customer(self):
        ...
        is_vip = input("¿Es un cliente VIP? (s/n): ").strip().lower()

        customer_info = f"{name} (DPI: {dpi})"

        if is_vip == "s":
            self.customer_service["vip"].enqueue(customer_info)
        else:
            self.customer_service["regular"].enqueue(customer_info)

        print("Cliente agregado con éxito.")

    def serve_customer(self):
        if not self.customer_service["vip"].is_empty():
            print(f"Atendiendo a cliente VIP: {self.customer_service['vip'].dequeue()}")
        elif not self.customer_service["regular"].is_empty():
            print(f"Atendiendo a cliente regular: {self.customer_service['regular'].dequeue()}")
        else:
